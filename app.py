...
def train_model():
    ...
    try:
        ...
        # נשתמש ב-n_estimators=100 (ברירת מחדל) ו max_depth=8 כדי לחסוך זמן ומשאבים ב-Render
        print("🤖 מאמן מודלי Random Forest...")
        model_dip = RandomForestRegressor(n_estimators=100, max_depth=8, random_state=42)
        model_peak = RandomForestRegressor(n_estimators=100, max_depth=8, random_state=42)
